...
namespace AoC_Point3D
{
    internal struct Point3D :
        ...
        IAdditionOperators<Point3D, Point, Point3D>,
        ISubtractionOperators<Point3D, Point, Point3D>,
        ...
        IComparisonOperators<Point3D, Point3D, bool>
    {
        ...
        public Point3D(Point xy, int z = 0)
        {
            _x = xy.X;
            _y = xy.Y;
            _z = z;
        ...
        public static Point3D operator -(Point3D left, Point3D right) => new(left.X - right.X, left.Y - right.Y, left.Z - right.Z);
        public static Point3D operator +(Point3D left, Point right) => new(left.X + right.X, left.Y + right.Y, left.Z);
        public static Point3D operator -(Point3D left, Point right) => new(left.X - right.X, left.Y - right.Y, left.Z);
        ...
        // IEqualityOperators<Point3D, Point, bool>
        public static bool operator ==(Point3D left, Point right) => left.X == right.X && left.Y == right.Y && left.Z == 0;
        public static bool operator !=(Point3D left, Point right) => !(left.X == right.X && left.Y == right.Y && left.Z == 0);

        ////IEqualityComparer
        public bool Equals(Point3D x, Point3D y) => x == y;
        ...
        // Hash Codes
        public override int GetHashCode() => unchecked(_x ^ _y ^ _z);
        public int GetHashCode([DisallowNull] Point3D obj) => GetHashCode();
