namespace AoC_2019_IntcodeVM
{
    ...
    internal class IntcodeVM
    {
        ...
        private readonly long[] _mainMemory = new long[8192];
        ...
        public State Run()
        {
            do
            {
                _currentState = Dispatcher(Ptr_ReadValue(Mode.Immediate));
            ...
            switch (instruction)
            {
                ...
                case 4: //output
                   ...
                   // if (_output == '\n') return State.Paused_For_Output;
                    //if (_outputBuffer.Count >= 3) return State.Paused_For_Output;
                    break;
        ...
        {
            output = '\0';
            if (_outputBuffer.TryPeek(out long result))
            {
                if (result > 'z') return false; // next value isn't a char. 
            }
        ...
        public void Reload(long relBase, long instPtr, long[] instructions)
        {
            Reset(instructions);

            _relativeBase = relBase;
            _instPtr = instPtr;
        }
