...
void calculateBridgeStrength(Dictionary<ulong, (int plugA, int plugB)> plugs, out int strongestBridge, out int longestBridgeStrength )
{

    // bridge contents, and open plug
    PriorityQueue<(ulong, int), int> queue = new();
    HashSet<ulong> queueContents = new();
    ...
    while (queue.Count > 0)
    {
        queue.TryDequeue(out (ulong bridge, int openPlug) q, out int bridgeStrength);
        ...
        {
            if ((q.bridge & currentPlug.Key) != 0) continue;
...
try
{
    ...
    Dictionary<ulong, (int plugA, int plugB)> plugs = new();
