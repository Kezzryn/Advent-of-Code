try
{
    ...
    HashSet<int> tape = [];
    Dictionary<int, (int nextValue, int nextState, int nextPOS)> nextStep = [];

    //Begin in state X.
    //Perform a diagnostic checksum after 1234 steps.
    int currentState = puzzleInput[0][0].ToCharArray().TakeLast(2).First();
    int numLoops =     int.Parse(puzzleInput[0][1].Split(' ')[5]);
