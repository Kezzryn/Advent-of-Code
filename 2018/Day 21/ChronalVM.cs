namespace AoC_2018_Day_21
{
    internal class ChronalVM
    {
        ...
        public List<int>? Run()
        {
            ...
            while (_registers[_boundRegister] < _instructions.Count)
            {
                Dispatcher(_instructions[_registers[_boundRegister]]);

                if(_registers[_boundRegister] == 29)
                {
                    if (returnValue.Contains(_registers[4])) return returnValue;
                    returnValue.Add(_registers[4]);
                }
